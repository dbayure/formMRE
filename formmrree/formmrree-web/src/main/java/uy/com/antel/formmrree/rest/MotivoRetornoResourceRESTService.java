...
@Path("/motivosRetorno")
@RequestScoped
public class MotivoRetornoResourceRESTService {
   ...
   public List<MotivoRetorno> listAll() {
      ...
      final List<MotivoRetorno> results = em.createQuery("select c from MotivoRetorno c order by c.id").getResultList();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Produces("application/json")
   public MotivoRetorno lookupById(@PathParam("id") long id) {
      return em.find(MotivoRetorno.class, id);
   }
}
