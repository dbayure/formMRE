...
@Path("/ocupaciones")
@RequestScoped
public class OcupacionResourceRESTService {
   ...
   public List<Ocupacion> listAll() {
      ...
      final List<Ocupacion> results = em.createQuery("select c from Ocupacion c order by c.id").getResultList();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Produces("application/json")
   public Ocupacion lookupById(@PathParam("id") long id) {
