...
//			return null;
		...
		try {
			...
			estadoCivil = mapper.readValue(new URL(  context.getExternalContext().getRequestScheme() + "://" + context.getExternalContext().getRequestServerName()
					+ ":"  + context.getExternalContext().getRequestServerPort() + context.getExternalContext().getRequestContextPath() 
					+ "/rest/estadosCiviles/" + value), EstadoCivil.class);
		}
		catch(Exception e) {
			throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error de Conversion", "Estado Civil no válido"));
		}
		return estadoCivil;
	}
