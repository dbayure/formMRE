...
@RequestScoped
public class VulnerabilidadResourceRESTService {
   ...
   public List<Vulnerabilidad> listAll() {
      ...
      final List<Vulnerabilidad> results = em.createQuery("select c from Vulnerabildad c order by c.id").getResultList();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Produces("application/json")
   public Vulnerabilidad lookupById(@PathParam("id") long id) {
      return em.find(Vulnerabilidad.class, id);
   }
}
