...
@Path("/paises")
@RequestScoped
public class PaisResourceRESTService {
   ...
   public List<Pais> listAll() {
      ...
      final List<Pais> results = em.createQuery("select c from Pais c order by c.id").getResultList();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Produces("application/json")
   public Pais lookupById(@PathParam("id") long id) {
