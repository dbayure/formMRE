...
@Path("/oficios")
@RequestScoped
public class OficioResourceRESTService {
   ...
   public List<Oficio> listAll() {
      ...
      final List<Oficio> results = em.createQuery("select c from Oficio c order by c.id").getResultList();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Produces("application/json")
   public Oficio lookupById(@PathParam("id") long id) {
